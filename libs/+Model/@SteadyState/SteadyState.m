...
classdef SteadyState < handle
    ...
    methods
        ...
        %% changeLoadFactor: 改变所有负荷的功率因数
        function changeLoadFactor(self, hysteresis)
            PM = [
                cos(hysteresis), -sin(hysteresis);
                sin(hysteresis), cos(hysteresis);
            ];
        ...
        function changeLoadFactorPerDeg(self, hysteresisDeg)
            ...
            % ];
            % [self.bus.Pd, self.bus.Qd] = PM*[self.bus.Pd, self.bus.Qd];
            self.changeLoadFactor(hysteresisDeg ./ 180 .* pi);
        end
